# Manuscript 도메인 계약

## 목적과 책임

Manuscript는 프로젝트의 장면과 원고 텍스트를 소유한다. 원고 내용의 삽입, 교체, 전체 갱신은 Manuscript의 규칙을 통해서만 수행한다.

## 보편 언어

- **원고:** 프로젝트에 속한 장면의 모음과 현재 활성 장면 상태
- **장면:** 장 번호, 제목, 본문, 관련 인물 및 세계관 참조를 가진 집필 단위
- **활성 장면:** 현재 집필 대상으로 선택된 장면
- **장면 추가:** 빈 장면을 원고 끝에 더하고 그 장면을 활성화하는 작업
- **장면 제목 수정:** 존재하는 장면의 제목을 정규화한 새 제목으로 바꾸는 작업
- **활성 장면 선택:** 존재하는 장면을 현재 집필 대상으로 바꾸는 작업
- **커서 위치:** 장면 본문에서 새 텍스트를 삽입할 문자 위치
- **텍스트 범위:** 시작 위치를 포함하고 끝 위치 직전까지를 나타내는 선택 구간
- **관련 문맥 참조:** 장면과 관련된 Story Bible 항목의 식별자
- **장면 초안 비교:** 같은 장면의 로컬 초안과 서버 초안을 줄 단위로 정렬해 차이를 확인하는 작업

## 핵심 모델

원고는 원고 식별자, 프로젝트 식별자, 장면 목록, 활성 장면 식별자로 구성된다. 장면은 장면 식별자, 제목, 장 번호, 본문, 관련 인물 식별자 목록, 관련 세계관 식별자 목록을 가진다.

## 불변 조건

- 원고는 하나의 프로젝트에 속해야 한다.
- 활성 장면 식별자는 원고에 존재하는 장면을 가리켜야 한다.
- 장면을 추가할 때 식별자는 비어 있지 않고 기존 장면 식별자와 중복되지 않아야 한다.
- 장면 제목을 수정하려면 해당 장면이 원고에 존재하고, 입력 제목은 앞뒤 공백을 제거한 뒤 비어 있지 않아야 한다.
- 장면 본문을 갱신하려면 해당 장면이 원고에 존재해야 한다.
- 커서 위치는 0 이상 본문 길이 이하여야 한다.
- 텍스트 범위는 `0 ≤ 시작 ≤ 끝 ≤ 본문 길이`를 만족해야 한다.
- 원고 변경은 입력 원고를 직접 변경하지 않고 변경된 새 원고를 반환해야 한다.
- 원고 텍스트는 Manuscript 이외의 도메인이 직접 변경해서는 안 된다.

## 유스케이스

### 초기 원고 생성

프로젝트 식별자를 받아 첫 장면과 그 장면을 가리키는 활성 장면 상태를 만든다.

### 장면 추가

비어 있지 않고 기존 장면과 중복되지 않는 식별자를 받아 새 장면을 만든다. 새 장면의 제목은 `제목 없는 장면`, 본문은 빈 문자열, 관련 인물과 세계관 참조는 각각 빈 목록이다. 장 번호는 기존 장면의 가장 큰 장 번호보다 1 큰 값이다. 새 장면을 기존 장면 목록 끝에 추가하고 그 장면을 활성 장면으로 설정한 새 원고를 반환하며, 입력 원고와 기존 장면은 변경하지 않는다.

### 활성 장면 선택

원고에 존재하는 장면 식별자를 받아 활성 장면이 다를 때는 활성 장면만 바꾼 새 원고를 반환하고, 이미 활성화된 장면이면 입력 원고를 그대로 반환한다. 장면 내용과 순서는 변경하지 않으며 입력 원고를 직접 변경하지 않는다. 존재하지 않는 장면은 선택할 수 없다.

### 장면 제목 수정

존재하는 장면 식별자와 새 제목을 받아 제목의 앞뒤 공백을 제거한 뒤 대상 장면의 제목만 변경한 새 원고를 반환한다. 정규화한 제목이 현재 제목과 같으면 입력 원고를 그대로 반환할 수 있다. 장면과 원고의 식별자, 활성 장면, 장 번호, 본문, 관련 인물 및 세계관 참조, 다른 장면과 입력 값은 변경하지 않는다. 존재하지 않는 장면이나 정규화한 뒤 비어 있는 제목은 전체 작업을 거부하며 부분 결과를 반환하지 않는다.

### 장면 본문 갱신

존재하는 장면의 본문 전체를 새 내용으로 교체한 원고를 반환한다.

### 커서 위치에 텍스트 삽입

유효한 커서 위치에 새 텍스트를 삽입한 본문을 반환한다.

### 선택 범위 교체

유효한 텍스트 범위를 새 텍스트로 교체한 본문을 반환한다.

### 장면 초안 비교

같은 장면의 두 버전을 입력받아 줄 단위 차이를 반환한다. 비교는 두 입력을 모두 변경하지 않으며, 장면 또는 원고 식별 관계가 일치하지 않으면 거부한다.

## 입력과 출력

- Projects에서 생성된 프로젝트 식별자를 입력받는다.
- Story Bible을 참조할 인물과 세계관 항목 식별자를 장면에 보관한다.
- Writing Assistant의 삽입 또는 교체 제안을 애플리케이션 유스케이스를 통해 입력받는다.
- 현재 장면 본문과 관련 문맥 참조를 집필 지원 요청 구성에 제공한다.

## 책임지지 않는 영역

- 프로젝트 메타데이터와 트로프 검증
- 인물 및 세계관 정보의 내용
- 집필 제안 생성
- 제안을 사용자에게 표시하고 적용 여부를 결정하는 과정
