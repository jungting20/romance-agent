...
분석 결과, 장면별 그래프 출처, 버전이 있는 프로젝트 지식 그래프 snapshot을
...
- **지식 그래프 출력:** 문서 요약과 인물·장소·사건·관계·이동·공통 참조·
  미해결 참조·모순을 공개 strict schema로 표현한 청크 단위 해석
- **일반 추출 항목:** confidence가 유한한 `0.8..1.0` 범위인 인물, 장소, 사건,
  관계, 이동 또는 공통 참조
- **미해결 참조:** 대상이 확정되지 않거나 confidence가 `0.8`보다 낮아 일반
  추출 항목으로 다룰 수 없는 표현과 가능한 대상
- **근거:** 청크 원문에 실제로 포함된 `evidence` 또는 `first_mention` 문자열.
  장면 절대 offset이나 별도의 영속 evidence 객체를 생성하지 않는다.
- **장면 그래프 레코드:** 프로젝트·장면 식별자, 장면 리비전·순서와 병합된
  `KnowledgeGraphOutput`을 보관하는 backend 저장 단위
- **프로젝트 지식 그래프 snapshot:** 현재 장면 그래프들에서 재구성한
  버전 있는 불변 v2 JSON 구조
- **source snapshot version:** agent가 분석 시작 시 읽은 프로젝트 snapshot 버전이며,
  backend 저장의 낙관적 동시성 기대값
...
- `KnowledgeGraphOutput`은 하나의 `Document`, `Entities` 안의 인물·장소·사건,
  관계, 이동, 공통 참조, 미해결 참조, 모순을 각각 불변 튜플로 가진다.
...
- `SceneGraphRecord`는 하나의 장면 리비전과 순서에 대한 병합 그래프를 해당
  프로젝트·장면 출처와 함께 보관한다.
- `ProjectKnowledgeGraphSnapshot`은 project ID, 0 이상의 snapshot version, v2 schema
  version, 장면별 `Document`, 통합된 인물·장소·사건·관계·이동·공통 참조,
  장면 출처를 보존한 미해결 참조와 모순을 가진다.
...
- 비어 있지 않은 `evidence`와 `first_mention`은 해당 청크의 불변 원문에
  그대로 포함되어야 한다.
...
- backend는 기존 프로젝트의 대표 이름과 별칭에 명확히 일치하는 청크 로컬 ID는
  기존 프로젝트 ID로 재사용한다. 여러 청크의 명확히 같은 신규 대상은 하나로
  통합하고, 애매한 대상은 별도로 유지하며 `POSSIBLE_SAME_AS` 관계를 보존한다.
  신규 ID는 원문 최초 등장 순서와 정규화 이름으로 결정적으로 할당한다.
...
- 재분석은 해당 project ID와 scene ID의 장면 그래프 레코드를 새 리비전·순서·
  그래프로 교체한다. 프로젝트 snapshot은 현재 장면 레코드 전체에서 매번
  재구성해 삭제되거나 교체된 장면의 이전 항목이 남지 않게 한다.
- 장면 출처는 `SceneGraphRecord`의 project ID, scene ID, 리비전과 순서로 보존한다.
  프로젝트 snapshot은 장면 순서대로 집계한 `documents`, unresolved references와
  contradictions를 포함한다.
- 빈 `ProjectKnowledgeGraphSnapshot`은 schema `project-knowledge-graph-snapshot-v2`, snapshot
  version `0`과 빈 모음을 가져야 한다. JSON codec은 v2만 canonical UTF-8 JSON으로
  인코딩·디코딩하고 v1, 알 수 없는 version, 추가 필드와 의미 불변 조건 위반을
  거부한다.
- 프로젝트 snapshot은 0 이상의 version과 장면 `chapter_id`의 유일성, 전역 graph
...
저장하며, 분석·병합·저장 실패 시 부분 상태를 게시하지 않는다.
