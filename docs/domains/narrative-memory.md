...
- `SceneGraphRecord`는 하나의 장면 리비전과 순서에 대한 미해결 참조·모순을
...
- `ProjectKnowledgeGraphSnapshot`은 project ID, 0 이상의 snapshot version, v2 schema
  ...
  사건·관계·이동·공통 참조·미해결 참조·모순의 평탄한 모음을 가진다.
...
- repository에 저장된 각 `SceneGraphRecord`가 자신의 미해결 참조와 모순을 포함한
  장면 출처의 권위 있는 소유자다. 출처는 레코드의 project ID, scene ID,
  리비전과 순서로 추적한다.
- 프로젝트 snapshot은 현재 장면 레코드의 값을 장면 순서대로 집계한
  `documents`, unresolved references와 contradictions를 포함하지만, 각 평탄한
