...
Story Bible 인물 카드는 Narrative Memory 내용을 저장하지 않는다. Narrative Memory는 필요한 이전
메모리의 소유자이자 제공자로서 Story Bible이 유지하는 durable `character_id`로 해당 메모리를
연결한다. 이 경계 명확화는 새 HTTP 작업이나 두 도메인 간 의존 방향 변경을 만들지 않는다.
