...
def _build_local_ids[Extracted: ExtractedEntity | ExtractedPlace](
    chunk: SceneChunk,
    kind: str,
    ...
    for extracted in extracted_values:
        ...
        result[extracted.local_ref] = _stable_id(chunk, kind, semantic_fields(extracted), evidence)
    return result
...
def _translate_relationship_event(
    ...
    subject_key = _resolve_reference(
        event.subject_ref, "entity", entity_refs, place_refs, known_entity_keys, known_place_keys
    )
    object_key = _resolve_reference(
        event.object_ref, "entity", entity_refs, place_refs, known_entity_keys, known_place_keys
    )
...
def _translate_location_event(
    ...
    character_key = _resolve_reference(
        event.character_ref, "entity", entity_refs, place_refs, known_entity_keys, known_place_keys
    )
    place_key = _resolve_reference(
        event.place_ref, "place", place_refs, entity_refs, known_place_keys, known_entity_keys
    )
    evidence = _translate_evidence(chunk, event.evidence)
...
def _translate_evidence(
    chunk: SceneChunk, evidence_values: tuple[RelativeEvidence, ...]
) -> tuple[Evidence, ...]:
    ...
    for evidence in evidence_values:
        ...
        translated.append(
            Evidence(
                ...
                chunk.start_offset + evidence.end_offset,
                evidence.text,
            )
        )
...
def _stable_id(
    chunk: SceneChunk, kind: str, semantic_fields: tuple[str, ...], evidence: tuple[Evidence, ...]
) -> str:
    payload = [
        CHUNK_ANALYSIS_SCHEMA_VERSION,
        chunk.scene_id,
