...
def _call() -> ChunkAnalysisCall:
    return ChunkAnalysisCall(
...
def test_strict_output_maps_to_provider_independent_internal_types() -> None:
    output = ChunkExtractionOutput.model_validate(_output_payload())

    assert map_chunk_extraction_output(output) == SceneChunkExtraction(
        summary="Alex arrived at home.",
        entities=(
            ExtractedEntity(
                local_ref="entity:alex",
...
def test_pydantic_ai_adapter_returns_validated_output_messages_usage_and_identity() -> None:
    ...
    adapter = PydanticAIChunkAnalyzer(model)

    result = asyncio.run(adapter.analyze(_call()))

    assert result.extraction == map_chunk_extraction_output(
        ChunkExtractionOutput.model_validate(_output_payload())
    )
